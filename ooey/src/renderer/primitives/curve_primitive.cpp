...
void CurvePrimitive::rebuild_geometry() const {
    ...
    auto x0 = static_cast<float>(p0_.x);
    auto y0 = static_cast<float>(p0_.y);
    auto x1 = static_cast<float>(p1_.x);
    auto y1 = static_cast<float>(p1_.y);

    auto cx1 = static_cast<float>(control1_.x);
    auto cy1 = static_cast<float>(control1_.y);
    auto cx2 = static_cast<float>(control2_.x);
    auto cy2 = static_cast<float>(control2_.y);
    ...
    for (int i = 0; i <= num_segments; ++i) {
        ...
        curve_points.emplace_back(static_cast<int>(std::round(x)), static_cast<int>(std::round(y)));
    }
