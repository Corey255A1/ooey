...
static void png_memory_read_callback(png_structp png, png_bytep data, png_size_t length) {
    auto* src = static_cast<PngMemorySource*>(png_get_io_ptr(png));
    if (!src) return;
    ...
    std::memcpy(data, src->data + src->offset, length);
    src->offset += length;
}

std::shared_ptr<Image> PngDecoder::decode_from_memory(const std::vector<uint8_t>& data) {
    if (data.size() < 8 || png_sig_cmp(data.data(), 0, 8) != 0) {
        return nullptr;
    }
