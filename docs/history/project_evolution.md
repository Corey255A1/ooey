...
## 30. Lexer, Parser and Code Generator Library (tooey)
To support development-time hot-reloading and ahead-of-time (AOT) C++ compilation of human-readable `.ooey` layout declarations:
- **Foundational tooey Library**: Created a dedicated `tooey` layout parsing and compilation library.
- **Lossless Lexer**: Built a custom C++ character-scanning lexer capable of mapping source text spans and offsets, tracking line/column boundaries, capturing space indentation, colons, inline/attribute assignments, strings, numerical arguments (with units like px, em, %), dynamic data bindings (`@binding.`), signals (`@signal.`), theme tokens (`@theme.`), and native placeholder AI blocks (`AI:`).
- **Indentation-based Parser**: Designed an indentation-based recursive scope parser using a node stack to build a standard layout Abstract Syntax Tree (AST) matching the EBNF grammar structure.
- **Code Generator**: Created an AOT compiler generator producing strongly-typed C++ class headers and implementation files. Resolves data bindings through native view model callbacks and connects signals directly, eliminating dynamic reflection or string dictionaries in production.
- **Unit Testing**: Added a comprehensive suite of unit tests validating basic tokenization, indentation nesting, AST attributes, and generated view constructors.

## 31. UI File Parser, Custom Component Checking, and Strongly-Typed Data Bindings
To satisfy the requirements of `docs/17-ooey-file-definition.md` for reusability and production bindings:
- **Offline AOT Compiler CLI**: Built a command-line tool `tooey_compiler` that takes `.ooey` layouts and generates standard `.hpp` and `.cpp` view classes.
- **Custom Sibling Component Resolution**: Enhanced the `Parser::parse()` mechanism to scan layout folders for other `.ooey` files. Sibling files (e.g. `FileMenu.ooey`) are resolved as custom reusable components, generating their respective constructor VM parameters, camel-to-snake VM getter calls, and automatic `#include` header generation.
- **Strongly-Typed Binding Operator (`<<=`)**: Implemented `operator<<=` in `tooey/binding.hpp` using compile-time template SFINAE. This binds viewmodel properties directly to widgets (like `Label`, `Button`, `TextBox`, and `ScrollBar`) using whichever setters they expose (`set_text()`, `set_label_text()`, or `set_value()`) at C++ native speed without string lookups.
- **Automated Reverse Binding**: Configured interactive layout nodes (like text boxes and scroll bars) to emit two-way reactive binding lambdas, updating viewmodel properties instantly on user interaction.
- **Comprehensive Unit Testing**: Added tests verifying custom component identification, header generation, constructorVM instantiation mapping, and the compile-time safety of the `<<=` operator.

## 32. Wayland Resource Cleanups and Code Generator VM Captures
During MemorySanitizer/AddressSanitizer profiling of the generated `hello_todo` executable:
- **Code Generator VM Scope Alignment**: Fixed an argument alignment issue in the recursive layout generation algorithm where child nodes did not receive the parent VM type, causing a build-time compiler warning. Correctly mapped all dynamic reverse bindings to capture and unlock `std::weak_ptr<ViewModel>` safely.
- **Wayland Seat and XDG Shell Teardown**: Patched a direct 192-byte client-side memory leak in the Wayland connection management backend (`WindowBackend::destroy`). Properly stored the registry-bound `xdg_wm_base` shell interface and the `wl_seat` instance, and invoked their respective destruction calls (`xdg_wm_base_destroy` and `wl_seat_destroy`) upon application exit.
- **Zero-Leak Validation**: Confirmed that all layout parsing applications, examples, and the core test suite compile and run cleanly with 0 sanitizer complaints or leaks detected at teardown.

## 33. First-Class CheckBox Control, Dynamic Row Templates, and View Composition Code Generation
To support list controls with custom compositions and interactive items (such as lists containing text, images, or checkboxes):
- **CheckBox Control**: Created `gooey::controls::CheckBox` in the standard controls set, rendering a rounded rect indicator and label, handling pressed pointer events, and supplying a bidirectional value interface.
- **Rearchitected ListControl**: Extended `ListControl` to support custom item views (`item_views_`) containing arbitrary hierarchies of elements. Re-routed layout arrangement, selection, and pointer/key events to dynamically delegate layout to child compositions and scroll heights accordingly.
- **Compiler Indented Template Parsing**: Updated the layout compiler (`tooey_compiler`) to recognize child scopes of `ListControl` as visual row templates. The list control's item height is automatically derived from the layout template's root height property.
- **Unified SFINAE Direct Setting (`set_control_value`)**: Introduced `tooey::set_control_value()` using compile-time template SFINAE to update row properties directly inside the factory loops without setting up full reactive bindings for every single cell.
- **Memory-Safe Captures**: Configured all generated template loops to decay lists using `std::decay_t` and `static_cast` for type resolution, and to capture the list control and view models as weak pointers (`weak_todoList`, `weak_viewModel`) to prevent circular reference cycles and leak-free lifetimes.
- **Unit Test Suite Expansion**: Added the `TooeyLayout.ListControlTemplateCodegen` test validating visual row template code generation, and confirmed that all 82 framework unit tests pass successfully.

## Summary
The current architecture of OOEY represents a modern, C++20 reactive UI framework. By starting with a solid abstraction layer, adopting a retained mode scene graph, structuring the codebase for modularity, layering a decoupled MVVM-C reactive system with dynamic property reflection, equipping it with DPI-aware auto-scaling, responsive layout controls, bidirectional scrolling, coordinate-mapped layout containers, and offering an AOT `.ooey` UI layout parsing and compilation suite with strongly-typed reactive bindings, OOEY provides a robust, explicit, and highly usable foundation for cross-platform UI development (including Linux desktop, WebAssembly/HTML5, and pure Android native hardware-accelerated platforms).